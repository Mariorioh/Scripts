...
def send_by_batch(str_list: List[str]):  # Sends a mango message containing at most 4 alerts each
...
def send_per_alert(str_list: List[str], mango_bot: MangoBot):
    for msg in str_list:
        Thread(target=mango_bot.send_msg, args=(msg,)).start()  # do sending to mango asynchronously
...
def main():
    ...
    while True:
        ...
        if alerts_str != '':
            ...
            send_per_alert(alerts_str_list, mango_bot)
        ...
        for alarm in alerts_handled:
            ...
            send_per_alert(tmp_list, mango_bot)
